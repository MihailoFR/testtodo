...
public class Tache {
    ...
    public Tache(String nom, String type, String note, String date) {
        this.nom = nom;
        this.type = type;
        this.note = note;
        this.date = date;
    ...
    public void ValiderTache() throws SQLException {
        ...
        PreparedStatement ajouter = maConnection.prepareStatement("INSERT INTO tache(nom,type,note,date,validite,personne,difficulte,ref_liste)" + "VALUES(?,?,?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
        ...
        ajouter.setInt(8, this.ref_liste);
    ...
    public void ModifierTache() throws SQLException {
        ...
        while (quitter.equals("m")) {
            ...
            int uid = oe.nextInt();
            System.out.println("Que voulez vous modifier (tout/nom/type/note/date/personne/difficulte/liste)?");
            ...
            if (reponse.equals("tout")) {
                ...
                System.out.println("Saisir le nouveau id de la liste : ");
                int upref_liste = update.nextInt();
                Connection maConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/projetliste?serverTimezone=UTC", "root", "");
                PreparedStatement modifier = maConnection.prepareStatement("Update tache set nom = ?,type = ?,note = ?,date = ?,personne = ?,difficulte = ?,ref_liste=? WHERE id_tache = ?");
                ...
                modifier.setInt(7, upref_liste);
                modifier.setInt(8, uid);
            ...
            if (reponse.equals("liste")) {
                System.out.println("Saisir le nouveau id de la liste : ");
                Scanner personne = new Scanner(System.in);
                int upref_liste = personne.nextInt();
                Connection maConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/projetliste?serverTimezone=UTC", "root", "");
                PreparedStatement modifier = maConnection.prepareStatement("Update tache set ref_liste = ? WHERE id_tache = ?");
                modifier.setInt(1, upref_liste);
                modifier.setInt(2, uid);
                modifier.executeUpdate();
                System.out.println("Modifiée");
                System.out.println("Voulez vous encore modifier ou quitter (m/q)");
    ...
    public void AfficherTache() throws SQLException {
        ...
        while (monResultat.next()) {
            System.out.println(monResultat.getInt("id_tache") + " - " + monResultat.getString("nom") + " - " + monResultat.getString("type") + " - " + monResultat.getString("note") + " - " + monResultat.getString("date") + " - " + monResultat.getString("difficulte") + " - " + monResultat.getString("validite") + " - " + monResultat.getString("personne") + " - " + monResultat.getInt("ref_liste"));
        }
    ...
    public void ValiditeDeLaTache() throws SQLException {
        ...
        if (reponses.equals("effectue")) {
            ...
            if (supresion.equals("o")) {
                Connection myConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/projetliste?serverTimezone=UTC", "root", "");
    ...
    public void tacheDansListe() throws SQLException {
        Connection maConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/todoliste?serverTimezone=UTC", "root", "");
        PreparedStatement requeteSelect = maConnection.prepareStatement("SELECT nom,ref_liste FROM tache");
        ResultSet monResultat = requeteSelect.executeQuery();

        while (monResultat.next()) {
            System.out.println(monResultat.getInt("nom") + " - " + monResultat.getString("ref_liste"));
        }
